...
# The cmd_elfconfig='true' override prevents Kbuild from executing target-mk_elfconfig (which fails on x86).
	...
	# Workaround: Execute build inside the pristine build tree instead of $(hdrdir) 
	# to completely avoid Kbuild falling back to $(CURDIR) and polluting host x86 tools.
	...
	cp -p $(builddir)/build-$*/scripts/basic/fixdep $(hdrdir)/scripts/basic/fixdep
	cp -p $(builddir)/build-$*/scripts/mod/modpost $(hdrdir)/scripts/mod/modpost
	cp -p $(builddir)/build-$*/scripts/genksyms/genksyms $(hdrdir)/scripts/genksyms/genksyms
